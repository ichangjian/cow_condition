...
public:
  ...
  {
    std::string cnt = getCurrentTime() + " COWCBC->" + m_file_name + ":" +
...
private:
  ...
  void save2file(const std::string &cnt, int flag = 0)
  {
    ...
    char buf[512];
#ifdef __ANDROID__
    strftime(buf, 64, "/sdcard/Download/log_CBC_%Y-%m-%d.txt", ltm);
#else
    strftime(buf, 64, "./temp/log_CBC_%Y-%m-%d.txt", ltm);
#endif
